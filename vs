...
alias y7='yt-dlp -S "res:720" $(wl-paste) ; nice nautilus /mnt/data/tmp/2/yt-dlp/'
y7f() { yt-dlp -S "res:720" $(wl-paste); for f in /mnt/data/tmp/2/yt-dlp/*; do cp "$f" "$(echo "$f" | sed 's/[:："＂?.№ —,;!&]/_/g' | sed 's/__/_/g' | sed 's/_webm/.web/')"; done; nice nautilus /mnt/data/tmp/2/yt-dlp/ ; }
alias y4='yt-dlp -S "res:480" $(wl-paste) ; nice nautilus /mnt/data/tmp/2/yt-dlp/'
alias y10='yt-dlp -S "res:1080" $(wl-paste) ; nice nautilus /mnt/data/tmp/2/yt-dlp/'
